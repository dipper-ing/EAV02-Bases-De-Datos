...
> de las migraciones Flyway (`V1` a `V5`) del repositorio principal.
...
Este sprint documenta las tres tablas que soportan las historias de usuario
de "Cimientos" que el backend tiene implementadas:
...
> verificación, estado de cuenta pendiente/activa, tabla de recuperación de
...
- **`app_users`**: cuenta de cada usuario de la plataforma. El campo `role`
  distingue entre `DEVELOPER` y `ADMIN` (no hay tabla de roles separada; es
  un valor controlado por `CHECK`). `email` y `username` son únicos de forma
  case-insensitive (índices sobre `lower(...)`).
- **`auth_sessions`**: registra cada sesión (refresh token) emitida al
  iniciar sesión. Relación **1 a N** con `app_users`: un usuario puede tener
  varias sesiones activas (por ejemplo, en distintos dispositivos).
- **`developer_profiles`**: perfil técnico opcional de un usuario. Relación
  **1 a 1** con `app_users` (`user_id` es `UNIQUE`): un usuario tiene, como
  máximo, un perfil técnico.

## 2. Preguntas de negocio clave

1. ¿Cuántos usuarios hay registrados por rol (`DEVELOPER` vs `ADMIN`)?
2. ¿Qué usuarios tienen la cuenta deshabilitada (`enabled = false`)?
...
- Índices: únicos case-insensitive en `app_users(email)` y
  ...
  búsquedas frecuentes (`user_id`, `expires_at`).
